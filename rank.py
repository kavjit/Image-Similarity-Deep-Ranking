"""
Created on Sun Oct 14 15:38:00 2018
...
trainset = ImageNetDataset(csv_file='file_table.csv',root_dir='tiny-imagenet-200/train/', transform = transformations)
...
#net = resnet18()
net = models.resnet101(pretrained = False)   #alternative way of loading the model
#resetting fully connected layer
num_ftrs = net.fc.in_features
net.fc = nn.Linear(num_ftrs, 4096)  #any number can be used
#loading my model
net.load_state_dict(torch.load('checkpoint.ckpt'))  #loading my previous checkpoint
...
for epoch in range(epochs):  
    ...
    for i, data in enumerate(trainloader):
        # get the inputs
        imgid, query, inclass, outclass = data    #this returns a seperate tensor for each image type containing 100 batches of each. Correct?      
    ...
    print(tim_cons)
    scheduler.step()
    torch.save(net.state_dict(),'checkpoint2.ckpt')
    torch.save(net,'full_model2.ckpt')
